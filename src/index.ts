...
class XrayReporter implements Reporter {
  ...
  onBegin(config: FullConfig, suite: Suite) {
    ...
    if (this.options.dryRun) console.log(`${bold(yellow(`⏺  `))}${bold(blue(`Starting a Dry Run with ${suite.allTests().length} tests`))}`);
    else console.log(`${bold(yellow(`⏺  `))}${bold(blue(`Starting the run with ${suite.allTests().length} tests`))}`);
    console.log(`${bold(blue(` `))}`);
  }
